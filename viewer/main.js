...
async function main() {
    ...
    const url = new URL(
        // "nike.splat",
        // location.href,
        params.get("run").concat("/splat/", params.get("epoch"), ".splat") || "example.splat",
        "http://".concat(window.location.host, "/logs/"),
    );
    ...
    const new_url = "http://".concat(window.location.host, "/logs/", params.get("run"), "/json/", params.get("epoch"), ".json") || "example.json";

    // Load the json file from the new_url and use the selectFile to load the json
    cameras = await fetch(new_url)
